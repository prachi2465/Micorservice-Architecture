package com.prachi.ApiGateway.controllers;
...
import com.prachi.ApiGateway.entities.AuthResponse;
...
public class AuthController {
	...
	public ResponseEntity<AuthResponse> login(@RegisteredOAuth2AuthorizedClient("okta") OAuth2AuthorizedClient client,
		    ...
		    Model model){
		//authorised==> what the user can access
		...
		List<String> list= oidcUser.getAuthorities().stream().map(gauth->{return gauth.getAuthority();}).collect(Collectors.toList());
